...
3. Write `.anchor/templates/plan.md` shape: Value, Priority, Slug, Preferred models,
   Depends on, Likely touch points, Progress, Goal, Context, Constraints, Steps,
   Risks, Done when. No `Lane:` / `Status:`. **Human-owned work:** add
   `- **Assignee:** <name|username|email>` (or `human`) for plans a person must
   complete — agents auto-skip claiming them but may still update status/comments.
   Absent or `ai` = agent-eligible (default).
...
5. **Populate `## Likely touch points` last too**, once Context read/Steps have
   surfaced real files/symbols — a short, deduped, whole-plan list (file, and a
   function/class name when known) so a lesser executor sees roughly where to
   look. Advisory/best-effort only — never invent a plausible file/symbol; write
   "not applicable" for a genuinely exploratory plan. Never changes what's
   enforced: the per-step `Touches` column and a task spec's `## Files in scope`
   (scope-gate-checked) stay authoritative.
6. Path only under `drafts/`. **Default filename `<slug>.local.md`** (private,
